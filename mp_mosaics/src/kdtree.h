...
class KDTree
{
  ...
  private:
    ...
    double getDistance(const Point<Dim>& point_1, const Point<Dim>& point_2) const;

    unsigned quickSelectIndex(vector<Point<Dim>>& kdTreeNodes, int dimension, unsigned leftNode, unsigned rightNode, unsigned pivotIndex);
    Point<Dim>& quickSelect(vector<Point<Dim>>& list, int dimension, unsigned leftNode, unsigned rightNode, unsigned k);

    KDTreeNode* constructorHelper(vector<Point<Dim>>& points, int dimension, unsigned leftNode, unsigned rightNode);
    
    // void createKDTree(int head, int end, int dimension); //to create the tree
    // void getMidpoint(int head, int end, int k, int dimension);
    // int getPivot(int head, int end, int pivot, int dimension);


    Point<Dim> findNearestNeighborHelper(KDTreeNode * subroot, const Point<Dim>& query, int dimension) const;
    Point<Dim> findNearestNeighbor(const Point<Dim>& query, KDTreeNode * node, int dimension) const;
    //copy constructor destructor helpers
    void destructorHelper(KDTreeNode * subroot);
    void copyConstructorHelper(KDTreeNode * currentRoot, KDTreeNode * otherRoot);
};
